...
function EQTWordmark({ size = 28, color = "white" }: { size?: number; color?: string }) {
  return (
    <span
      style={{
        fontFamily: "var(--fs-font-body)",
...
function FleetSyncWordmark({ size = 20, weight = 600 }: { size?: number; weight?: number }) {
  return (
    <span style={{ fontSize: size, fontWeight: weight, letterSpacing: -0.5, color: "white" }}>
...
function ModuleCard({
  icon, color, title, desc, status, hoverColor,
}: {
  icon: string;
  ...
  const statusMap = {
    live: { bg: "rgba(29,158,117,0.15)", text: "var(--fs-teal-light)", label: "LIVE" },
    building: { bg: "var(--fs-magenta-glow)", text: "var(--fs-magenta-bright)", label: "BUILDING" },
    planned: { bg: "rgba(138,153,180,0.15)", text: "var(--fs-text-muted)", label: "PLANNED" },
  };
      ...
      <div className="fs-page">
        ...
        <nav className="fs-nav">
          ...
          <ul className="fs-nav-links">
            ...
            <li><a href="#">Docs</a></li>
            <li>
              <a href="/api/login" className="fs-nav-cta" data-testid="link-login">
                Sign in →
              </a>
            </li>
          </ul>
        ...
        <div className="fs-eqt-strip">
          ...
          <div className="fs-eqt-strip-text">
            ...
            <br />
            America's largest natural gas producer — scale, reliability, and operational excellence.
          </div>
        </div>
          ...
          {stats.map((s, i) => (
            <div className="fs-stat" key={i}>
              <div className="fs-stat-num">
                {s.value}
                {s.suffix && (
                  <span style={{ fontFamily: "var(--fs-font-body)", fontSize: 16, color: "var(--fs-text-muted)" }}>
                    {s.suffix}
                  </span>
                )}
              </div>
        ...
        <section className="fs-modules" id="modules">
          ...
          <h2 className="fs-section-title">Every operations surface, one system of record</h2>
        ...
        <section className="fs-preview">
          ...
          <div className="fs-preview-container">
            <div className="fs-preview-header">
              ...
              <span className="fs-preview-url">app.fleetsync.eqt.com / executive-dashboard</span>
            </div>
            <div className="fs-preview-body">
              <div className="fs-preview-sidebar">
                ...
                {sidebarItems.map((item, i) => (
                  <div key={i} className={`fs-preview-sidebar-item${item.active ? " active" : ""}`}>
              ...
              <div className="fs-preview-main">
                <div className="fs-preview-topbar">
                  <div className="fs-preview-topbar-title">Executive dashboard</div>
                  <div className="fs-preview-topbar-filters">
                    {["Baseline", "Forecast", "Actual"].map((f, i) => (
                      <span key={f} className={`fs-preview-filter-pill${i === 0 ? " active" : ""}`}>{f}</span>
                    ))}
                  </div>
                </div>
                <div className="fs-preview-kpi-row">
                  {kpis.map((k, i) => (
                    <div className="fs-preview-kpi" key={i}>
                      ...
                      <div className={`fs-preview-kpi-delta ${k.up ? "up" : "dn"}`}>{k.delta}</div>
                    </div>
                  ))}
                </div>
                <div className="fs-preview-chart">
                  <div className="fs-preview-chart-header">
                    <div className="fs-preview-chart-title">7-day delivery trend</div>
                    <div className="fs-preview-chart-legend">
                      {[
                        ...
                        <div className="fs-legend-item" key={l.label}>
                          <div className="fs-legend-dot" style={{ background: l.color }} />
                          {l.label}
                        </div>
                      ))}
                    </div>
                    ...
                    {chartData.map((d, i) => (
                      <div className="fs-chart-col-wrap" key={i}>
                        <div className="fs-chart-col">
                          <div className="fs-chart-bar" style={{ background: "var(--fs-magenta)", opacity: 0.5, height: animate ? `${Math.round((d.c / maxVal) * 65)}px` : "0px", transitionDelay: `${i * 100}ms` }} />
                          <div className="fs-chart-bar" style={{ background: "var(--fs-blue)", height: animate ? `${Math.round((d.d / maxVal) * 65)}px` : "0px", transitionDelay: `${i * 100 + 40}ms` }} />
                          <div className="fs-chart-bar" style={{ background: "var(--fs-teal)", opacity: 0.6, height: animate ? `${Math.max(4, Math.round((d.s / maxVal) * 65))}px` : "0px", transitionDelay: `${i * 100 + 80}ms` }} />
                        </div>
        ...
        <div className="fs-powered">
          ...
          <div className="fs-powered-logos">
            {["Databricks", "Gemini Tickets", "Glancer", "Jarvis BI"].map((name) => (
              <span className="fs-powered-item" key={name}>{name}</span>
            ))}
          </div>
        </div>
...
.fs-nav-links { display: flex; align-items: center; gap: 2rem; list-style: none; }
.fs-nav-links a {
...
.fs-nav-cta {
  ...
  padding: 8px 20px; border-radius: 6px; font-weight: 600; transition: background 0.2s;
}
...
.fs-btn-secondary:hover { border-color: var(--fs-border-active); background: var(--fs-surface); }
...
.fs-stat-num { font-family: var(--fs-font-display); font-size: 32px; color: white; margin-bottom: 4px; }
.fs-stat-label { font-size: 12px; color: var(--fs-text-muted); text-transform: uppercase; letter-spacing: 1.5px; }
...
.fs-module-grid {
  ...
  border: 0.5px solid var(--fs-border); border-radius: 12px; overflow: hidden;
}
.fs-module-card {
...
.fs-preview-sidebar { padding-right: 1.5rem; border-right: 0.5px solid var(--fs-border); }
.fs-preview-sidebar-logo { display: flex; align-items: center; gap: 10px; padding: 8px 12px; margin-bottom: 16px; }
...
.fs-preview-sidebar-item.active { background: var(--fs-magenta-glow); color: var(--fs-magenta-bright); }
...
.fs-preview-filter-pill {
  ...
  border: 0.5px solid var(--fs-border); color: var(--fs-text-muted); background: transparent;
}
.fs-preview-filter-pill.active {
  border-color: var(--fs-magenta); color: var(--fs-magenta-bright); background: var(--fs-magenta-glow);
}
.fs-preview-kpi-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 1.25rem; }
.fs-preview-kpi {
  background: rgba(255,255,255,0.03);
  border: 0.5px solid var(--fs-border); border-radius: 8px; padding: 12px 14px;
}
.fs-preview-kpi-label { font-size: 10px; color: var(--fs-text-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px; }
.fs-preview-kpi-val { font-family: var(--fs-font-display); font-size: 22px; color: white; }
...
.fs-preview-chart {
  ...
  padding: 1rem 1.25rem; height: 140px; position: relative; overflow: hidden;
}
.fs-preview-chart-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
.fs-preview-chart-title { font-size: 12px; color: var(--fs-text-muted); }
.fs-preview-chart-legend { display: flex; gap: 12px; }
.fs-legend-item { display: flex; align-items: center; gap: 5px; font-size: 10px; color: var(--fs-text-muted); }
.fs-legend-dot { width: 8px; height: 8px; border-radius: 2px; }
.fs-chart-bars { display: flex; align-items: flex-end; gap: 4px; height: 85px; }
.fs-chart-col-wrap { flex: 1; display: flex; flex-direction: column; align-items: center; }
.fs-chart-col { width: 100%; display: flex; gap: 2px; align-items: flex-end; height: 70px; }
.fs-chart-bar { flex: 1; border-radius: 2px 2px 0 0; transition: height 0.8s cubic-bezier(0.23, 1, 0.32, 1); }
.fs-chart-day-label { font-size: 10px; color: var(--fs-text-muted); margin-top: 6px; text-align: center; }

.fs-powered {
  padding: 2rem 2.5rem 3rem; display: flex; align-items: center; gap: 3rem;
  border-top: 0.5px solid var(--fs-border);
}
.fs-powered-label { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: var(--fs-text-muted); white-space: nowrap; }
.fs-powered-logos { display: flex; align-items: center; gap: 2.5rem; flex: 1; }
.fs-powered-item { font-size: 13px; font-weight: 500; color: rgba(138,153,180,0.4); letter-spacing: 0.5px; }

.fs-footer {
  padding: 2rem 2.5rem; border-top: 0.5px solid var(--fs-border);
  display: flex; align-items: center; justify-content: space-between;
}
.fs-footer-left { display: flex; align-items: center; gap: 16px; }
.fs-footer-sep { width: 1px; height: 16px; background: rgba(255,255,255,0.1); }
